...
lca_model = MNLogit(y5, X)
lca_results = lca_model.fit()

# Extract latent class probabilities
latent_classes = lca_results.predict(X)

X = np.concatenate((X, latent_classes),axis=1)

X[np.isnan(X)] = 0
X[np.isinf(X)] = 0

# stepmix method
model = StepMix(n_components=2, measurement="categorical")
model.fit(X)

# Extract latent class probabilities
latent_classes = model.predict(X)

latent_classes=latent_classes[:, np.newaxis]

print(np.shape(X))
print(latent_classes)
print(np.shape(latent_classes))
X = np.concatenate((X, latent_classes),axis=1)
...
aFeatures=np.unique(np.hstack([aFeatures0.flatten(), aFeatures1.flatten(), aFeatures3.flatten(), aFeatures4.flatten(), aFeatures5.flatten()])).flatten()
